...
public interface UserAnswerRepository extends JpaRepository<UserAnswer, Long> {
        ...
        @Query("SELECT ua.question.id FROM UserAnswer ua WHERE ua.user.id = :userId")
        List<Long> findQuestionIdsByUserId(@Param("userId") Long userId);

        boolean existsByUserIdAndQuestionId(Long userId, Long questionId);

        @Query("SELECT ua FROM UserAnswer ua JOIN ua.question q WHERE ua.userId = :userId")
        List<UserAnswer> findByUserIdWithQuestions(Long userId, String search, Pageable pageable);
